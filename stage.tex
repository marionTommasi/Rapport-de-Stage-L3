...
\Fn{\Fct($start$, $end$, $cpt$, $pos$)}{
	\KwData{Les arguments : \\
		\begin{itemize}
			...
			\item $cpt$ et $pos$, un entier et un ensemble initialement nul et vide, utilisés dans les calculs;
		\end{itemize}\\
...
\caption{Recherche de positions dans le texte d'un intervalle}	
\label{bloc}	
\end{algorithm}
